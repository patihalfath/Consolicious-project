...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['paymentProof']) && $_FILES['paymentProof']['error'] == 0) {
        ...
        if (move_uploaded_file($fileTmpPath, $uploadFile)) {
            ...
            // Hitung total harga
            $total_price = $prices[$xboxType] * $duration * $quantity;

            // Menyusun query untuk menyimpan data transaksi
            $sql = "INSERT INTO sewa_xbox (product_name, quantity, rental_date, total_price, user_name, user_phone, user_address, payment_method, bukti_pembayaran)
                    VALUES (?, ?, NOW(), ?, ?, ?, ?, ?, ?)";
            
            // Menyiapkan statement
            $stmt = $conn->prepare($sql);
            // Pastikan data sesuai dengan tipe yang diterima oleh tabel (parameter binding)
            $stmt->bind_param("siisssss", $xboxType, $quantity, $total_price, $name, $phone, $address, $method, $uploadFile);
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $sql .= " WHERE Xbox_jenis LIKE '%$search%'";
}
...
<body>
    ...
    <header>
        ...
        <form action="xbox.php" method="GET" class="search-form">
            <input type="text" name="search" placeholder="Search Xbox..." 
    ...
    <main>
        <section class="games">
            <h2>Pilih Xbox yang Kamu Inginkan!</h2>
            <div class="game-grid">
            <?php
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo '<div class="game-item">
                        <img src="' . $row['image_path'] . '" alt="' . $row['Xbox_jenis'] . '" />
                        <h3>' . $row['Xbox_jenis'] . '</h3>
                        <p>' . $row['deskripsi'] . '</p>
                        <p>Stok Tersedia: ' . $row['stok'] . ' unit</p>
                        <a href="javascript:void(0);" 
                           onclick="openPaymentForm(\'' . $row['Xbox_jenis'] . '\')">
    ...
    <div id="paymentFormModal" class="payment-form-modal">
        <div class="payment-form-container">
            ...
            <form id="paymentForm" method="POST" action="ps.php" enctype="multipart/form-data">
                ...
                <label for="xboxType">Jenis Xbox</label>
                <input type="text" id="xboxType" name="xboxType" readonly required>
                ...
                <button type="button" id="submitButton" onclick="showQrCode()">Kirim Pembayaran</button><br>
                <button type="submit" id="continueButton" style="display:none;">Lanjut</button>
    ...
    <script>
        // Daftar harga per jenis PlayStation
        const prices = {
            'Xbox Series X': 12000,  // Harga per jam untuk PS5
            'Xbox Series S': 8000,   // Harga per jam untuk PS4
            'Xbox One X': 6000    // Harga per jam untuk PS3
        };

        // Open the payment form modal
        function openPaymentForm(Type) {
            // Isi field Jenis PS secara otomatis
            document.getElementById("xboxType").value = xboxType;

            // Menampilkan harga per jam sesuai dengan jenis PS yang dipilih
            const price = prices[xboxType] || 0;
        ...
        function showQrCode() {
            ...
            const xboxType = document.getElementById("xboxType").value;
            const quantity = document.getElementById("quantity").value;
            const duration = document.getElementById("duration").value;
            const method = document.getElementById("method").value;

            // Check if all fields are filled
            if (name && phone && address && xboxType && quantity && duration && method) {
                document.getElementById("errorMessage").style.display = "none";

                const totalPrice = prices[xboxType] * duration * quantity;

                // Generate a simple payment link (can be replaced with actual payment gateway URL)
                const paymentLink = "https://example.com/payment?amount=" + totalPrice;

                // Generate the QR code URL using a free API (for demo purposes)
