...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['paymentProof']) && $_FILES['paymentProof']['error'] == 0) {
        ...
        if (move_uploaded_file($fileTmpPath, $uploadFile)) {
            // Daftar harga per jenis switch
            $prices = [
                'Switch OLED' => 25000,  // Harga per jam untuk switch5
                'Switch LITE' => 20000,   // Harga per jam untuk switch4
                'Switch 2' => 15000    // Harga per jam untuk switch3
            ];

            // Hitung total harga
            $total_price = $prices[$switchType] * $duration * $quantity;

            // Menyusun query untuk menyimpan data transaksi
            $sql = "INSERT INTO sewa_switch (product_name, quantity, rental_date, total_price, user_name, user_phone, user_address, payment_method, bukti_pembayaran)
                    VALUES (?, ?, NOW(), ?, ?, ?, ?, ?, ?)";
            
            // Menyiapkan statement
            $stmt = $conn->prepare($sql);
            // Pastikan data sesuai dengan tipe yang diterima oleh tabel (parameter binding)
            $stmt->bind_param("siisssss", $switchType, $quantity, $total_price, $name, $phone, $address, $method, $uploadFile);
...
// Query untuk mengambil data switch
$sql = "SELECT * FROM switch";
if (isset($_GET['search']) && !empty($_GET['search'])) {
    $search = $conn->real_escape_string($_GET['search']);
    $sql .= " WHERE switch_jenis LIKE '%$search%'";
}
...
<body>
    ...
    <header>
        ...
        <form action="switch.php" method="GET" class="search-form">
            <input type="text" name="search" placeholder="Search switch..." 
    ...
    <main>
        <section class="games">
            <h2>Pilih switch yang Kamu Inginkan!</h2>
            <div class="game-grid">
            <?php
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo '<div class="game-item">
                        <img src="' . $row['image_path'] . '" alt="' . $row['switch_jenis'] . '" />
                        <h3>' . $row['switch_jenis'] . '</h3>
                        <p>' . $row['deskripsi'] . '</p>
                        <p>Stok Tersedia: ' . $row['stok'] . ' unit</p>
                        <a href="javascript:void(0);" 
                           onclick="openPaymentForm(\'' . $row['switch_jenis'] . '\')">
    ...
    <div id="paymentFormModal" class="payment-form-modal">
        <div class="payment-form-container">
            ...
            <form id="paymentForm" method="POST" action="switch.php" enctype="multipart/form-data">
                ...
                <label for="switchType">Jenis switch</label>
                <input type="text" id="switchType" name="switchType" readonly required>
    ...
    <script>
        ...
        function openPaymentForm(switchType) {
            // Isi field Jenis switch secara otomatis
            document.getElementById("switchType").value = switchType;

            // Menampilkan harga per jam sesuai dengan jenis switch yang dipilih
            const price = prices[switchType] || 0;
    ...
    const psType = document.getElementById("switchType").value;
